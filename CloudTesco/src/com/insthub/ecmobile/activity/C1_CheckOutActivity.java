...
public class C1_CheckOutActivity extends BaseActivity implements OnClickListener, BusinessResponse {
	...
	protected void onCreate(Bundle savedInstanceState) {		
        ...
        totalPriceTextView = (TextView) findViewById(R.id.accounts_payable);
		submit = (Button) findViewById(R.id.balance_submit);
	...
	public void setInfo() {
		...
		phoneNum.setText(shoppingCartModel.address.mobile);
//		phoneNum.setText(shoppingCartModel.address.tel);
		...
		for(int i=0;i<shoppingCartModel.balance_goods_list.size();i++)
        {
			...
			if (imageType.equals("high")) {
				imageLoader.displayImage(shoppingCartModel.balance_goods_list.get(i).img.thumb, goods_img, EcmobileApp.options);
			} else if (imageType.equals("low")) {
				imageLoader.displayImage(shoppingCartModel.balance_goods_list.get(i).img.small, goods_img, EcmobileApp.options);
			} else {
            ...
            view.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View arg0) {
					...
					Intent it = new Intent(C1_CheckOutActivity.this, B2_ProductDetailActivity.class);
			    	it.putExtra("good_id", shoppingCartModel.balance_goods_list.get(index).goods_id);
			    	C1_CheckOutActivity.this.startActivity(it);
				}
			});
		}

        totalPriceTextView.setText(getBaseContext().getResources().getString(R.string.accounts_payable) + "￥" + totalGoodsPrice);
...
//                                showAlipayDialog();
    ...
    void refreshTotalPrice()
    {
        ...
        totalPriceTextView.setText(getBaseContext().getResources().getString(R.string.accounts_payable) + "￥"+total_price_show);
    }
