...
public class TimeUtil {
    ...
    /**
     * @param timeStr   结束的时间
     * @param Need    需要时间的类型    天， 小时， 分， 秒，    注意返回的不是距离结束时间的   总秒数，
     * @return
     */
	public static String getFromEndWhenVehicle (String timeStamp, String Need){
        Date currentTime = new Date();
        long currentTimeStamp = currentTime.getTime();

        long total_seconds = (Long.parseLong(timeStamp) * 1000L - currentTimeStamp)/1000;
        ...
        else if(Need.equals("时"))
        {
        	leftTime = "" + hours;
        }
        else if(Need.equals("分"))
        {
        	leftTime = "" + minutes;
        }
        else if(Need.equals("秒"))
        {
         	leftTime = "" + seconds;
        }
        else 
